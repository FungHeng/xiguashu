...
def test_RandomForestRegressor(*data):
    ...
    print("Residual sum of squares: %.5f" % metrics.mean_absolute_error(y_test,regr.predict(X_test)))
...
def test_RandomForestRegressor_max_depth(*data):
    ...
    for max_depth in maxdepths:
        regr=ensemble.RandomForestRegressor(max_depth=max_depth,n_estimators=100)
        regr.fit(X_train,y_train)
        training_scores.append(regr.score(X_train,y_train))
        testing_scores.append(regr.score(X_test,y_test))
...
def test_RandomForestRegressor_max_features(*data):
   ...
   测试 RandomForestRegressor 的预测性能随  max_features 参数的影响
    ...
    for max_feature in max_features:
        regr=ensemble.RandomForestRegressor(max_features=max_feature,n_estimators=100)
        regr.fit(X_train,y_train)
        training_scores.append(regr.score(X_train,y_train))
        testing_scores.append(regr.score(X_test,y_test))
