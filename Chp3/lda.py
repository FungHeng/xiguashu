...
def plot_LDA(converted_X,y):
    ...
    for target,color,marker in zip([0,1,2],colors,markers):#zip()方法用在for循环中，支持并行迭代
        pos=(y==target).reshape(-1)#由标签转换为bool矩阵，分成不同标签类型的矩阵(150,) ravel平铺用于拆分数据# pos=(y==target).ravel()
        X=converted_X[pos,:]
...
def run_plot_LDA():
    ...
    # X=np.vstack((X_train,X_test))#垂直（按照行顺序）的把数组给堆叠起来[ [] [] ]
    ...
    converted_X=np.dot(X,np.transpose(lda.coef_))#+lda.intercept_ ##可加可不加
