...
def test_DecisionTreeClassifier(*data):
    ...
    #画决策树图
    dot_data = export_graphviz(clf, out_file=None, filled=True, impurity=True, rounded=True, special_characters=True)
    graph = pydotplus.graph_from_dot_data(dot_data)
    graph.write_pdf('tree1.pdf')  # or png
...
def test_DecisionTreeClassifier_leaf_nodes(*data,max_leaf_nodes):
    '''
    测试 DecisionTreeClassifier 的预测性能随 max_depth 参数的影响
    ...
    '''
    X_train,X_test,y_train,y_test=data
    leaf_nodes=np.arange(1,max_leaf_nodes)
    training_scores=[]
    testing_scores=[]
    for nodes in leaf_nodes:
        clf = DecisionTreeClassifier(max_depth=nodes)
        clf.fit(X_train, y_train)
        training_scores.append(clf.score(X_train,y_train))
        testing_scores.append(clf.score(X_test,y_test))
    ...
    fig=plt.figure()
    ax=fig.add_subplot(1,1,1)
    ax.plot(leaf_nodes,training_scores,label="traing score",marker='o')
    ax.plot(leaf_nodes,testing_scores,label="testing score",marker='*')
...
if __name__=='__main__':
    ...
    # test_DecisionTreeClassifier_depth(X_train,X_test,y_train,y_test,maxdepth=20) # 调用 test_DecisionTreeClassifier_depth
    # test_DecisionTreeClassifier_leaf_nodes(X_train,X_test,y_train,y_test,max_leaf_nodes=15)