...
def creat_data(n):
    ...
    print(y)
    noise_num=n
    y += 3 * (0.5 - np.random.normal(0,0.01,n)) # 每第5个样本，就在该样本的值上添加噪音
...
def DecisionTreeRegressor_depth_plot(*data,maxdepth):
    '''不同maxdepth的决策树的复杂度'''
    ...
    for depth in maxdepth:
        ...
        regr.fit(X_train, y_train)
        Y = regr.predict(X)
        ax.plot(X, Y, label="predict_value.max_depth=%s"%depth, linewidth=2, alpha=0.5)
    ax.set_xlabel("data")
    ax.set_ylabel("target")
    ax.set_title("Decision Tree Regression")
    ax.legend(framealpha=0.5)
...
if __name__=='__main__':
    ...
    DecisionTreeRegressor_depth_plot(X_train,X_test,y_train,y_test,maxdepth=[1,3,7])