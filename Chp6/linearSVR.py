...
def test_LinearSVR(*data):
    ...
    regr=svm.LinearSVR(max_iter=10000)
    regr.fit(X_train,y_train)
    print('Coefficients:%s, intercept %s'%(regr.coef_,regr.intercept_))
    print('Score: %.5f' % regr.score(X_test, y_test))
    print("Residual sum of squares: %.5f" % metrics.mean_squared_error(y_test,regr.predict(X_test)))
...
def test_LinearSVR_epsilon(*data):
    ...
    for  epsilon in  epsilons:
        regr=svm.LinearSVR(epsilon=epsilon,loss='epsilon_insensitive')
        regr.fit(X_train,y_train)
        train_scores.append(regr.score(X_train, y_train))
        test_scores.append(regr.score(X_test, y_test))
        MSE.append(metrics.mean_squared_error(y_test, regr.predict(X_test)))
    print(MSE)
    fig=plt.figure()
    ax=fig.add_subplot(1,1,1)
    ax.plot(epsilons,train_scores,label="Training score ",marker='+' )
    ax.plot(epsilons,test_scores,label= " Testing  score ",marker='o' )
    ax.plot(epsilons,MSE,label= " Mean Squared Error ",marker='*' )
...
def test_LinearSVR_C(*data):
    ...
    MSE=[]
    for  C in  Cs:
        regr=svm.LinearSVR(epsilon=0.1,loss='squared_epsilon_insensitive',C=C,max_iter=2000)
        regr.fit(X_train,y_train)
        train_scores.append(regr.score(X_train, y_train))
        test_scores.append(regr.score(X_test, y_test))
        MSE.append(metrics.mean_squared_error(y_test, regr.predict(X_test)))
    ...
    ax.plot(Cs,MSE,label= " Mean Squared Error ",marker='*' )
