...
def plot_samples(ax,samples):
    ...
    ax.scatter(samples[Y==1,0],samples[Y==1,1],
        samples[Y==1,2],marker='+',label='+',color='b') # 绘制正类样本点
    ...
    :return: 一个元组，依次为：最终的权重向量，最终的 b 值，迭代次数
     ...
     while True:
        ...
        while(i< length): ## 遍历一轮样本集中的所有的样本点
            step_num+=1
            '''
          当应用于线性不可分数据集时，用下面4行代替上面的 step_num+=1 这一行。如果不这么做，那么当用于线性不可分数据集时，迭代永远不会停止。
            step_num+=1
            if step_num>=1000000:
...
def test_eta(data,ax,etas,w_0,alpha_0,b_0):
        ...
        nums2=[]
        for eta in etas:
            num_1=perceptron(data,eta,w_0=w_0,b_0=b_0) # 获取原始形式算法的迭代次数
            num_2=perceptron_dual(data,eta=0.1,alpha_0=alpha_0,b_0=b_0) # 获取对偶形式算法的迭代次数
            nums1.append(num_1)
            nums2.append(num_2)
